...
import business.recommendation.core.api.UserInterestRecommenderApi;
...
@Service
public class UserInterestRecommenderApiImpl implements UserInterestRecommenderApi {
    ...
    public ArrayList<RecmdMissionEntity> recommend(Long workerId) {
        ...
        if(recmdInterestSimWorker.size()==0){
            return new ArrayList<RecmdMissionEntity>();
        }
        else{
            //找到所有符合任务
            Map<Long,Double> missions=new HashMap<>();
            for(int i=0;i<recmdInterestSimWorker.size();i++){
                SimWorkerEntity simWorkerEntity=recmdInterestSimWorker.get(i);
